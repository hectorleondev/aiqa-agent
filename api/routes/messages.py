...
def create_message(
    ...
    return repo.create(
        issue_key=message.issue_key, body=message.body, status=message.status
    )
...
def get_messages_by_issue(
    issue_key: str, repo: MessageRepository = Depends(get_message_repository)
):
...
def update_message_status(
    ...
    message = repo.update_status(message_id, update.status, update.body)
    if not message:
        raise HTTPException(status_code=404, detail="Message not found")
    return message
